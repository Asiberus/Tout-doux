    ...
    <v-card>
        ...
        <v-card-text>
            <v-form ref="form" v-model="sectionForm.valid" @submit.prevent="emitSubmitEvent">
                <v-row>
                    <v-col>
                        <v-text-field v-model="sectionForm.data.name" label="Name" counter="50" maxlength="50" required
                        :rules="sectionForm.rules.name" autofocus>
                        </v-text-field>
                    </v-col>
                </v-row>
                <v-card-actions class="d-flex justify-end mt-3">
                    <v-btn color="success" small :disabled="!sectionForm.valid" @click="emitSubmitEvent">
