<script setup lang="ts">
import { Tag, TagForm, TagType } from '@/models/tag.model'
import TagDialog from '@/views/components/tag/TagDialog.vue'
import EmptyListDisplay from '@/components/EmptyListDisplay.vue'
import TagCard from '@/views/components/tag/TagCard.vue'
import { tagApi } from '@/api'
import { getDialogWidth } from '@/utils/dialog.utils'
import { onBeforeMount, ref, watch } from 'vue'
import { useDisplay } from 'vuetify'

const display = useDisplay()

const props = defineProps<{
  type: TagType
}>()

defineExpose({ openTagDialog })

onBeforeMount(() => {
  fetchTagList()
})

const tagList = ref<Tag[]>([])
const tagDialog = ref(false)

watch(
  () => props.type,
  () => fetchTagList()
)

function fetchTagList(): void {
  tagApi
    .getTagList({ size: 0, type: props.type })
    .then(response => (tagList.value = response.content))
    .catch(error => console.error(error))
}

function openTagDialog(): void {
  tagDialog.value = true
}

function createTag(data: TagForm): void {
  tagApi
    .createTag(data)
    .then(response => {
      tagList.value.push(response)
      tagDialog.value = false
    })
    .catch(error => console.error(error))
}

function updateTag({ id, data }: { id: number; data: TagForm }): void {
  tagApi
    .updateTag(id, data)
    .then(response => {
      const index = tagList.value.findIndex(tag => tag.id === id)
      if (index !== -1) tagList.value.splice(index, 1, response)
    })
    .catch(error => console.error(error))
}

function deleteTag(id: number): void {
  tagApi
    .deleteTag(id)
    .then(() => {
      const index = tagList.value.findIndex(tag => tag.id === id)
      if (index !== -1) tagList.value.splice(index, 1)
    })
    .catch(error => console.error(error))
}
</script>

<template>
  <div class="fill-height d-flex flex-column">
    <template v-if="tagList.length > 0">
      <div class="tag-wrapper">
        <TagCard
          v-for="tag of tagList"
          :key="tag.id"
          :tag
          @update="updateTag"
          @delete="deleteTag" />
      </div>
    </template>
    <template v-else>
      <EmptyListDisplay
        :message="`You didn't configure any ${type} tag yet.`"
        class="empty-list-display">
        <template #img>
          <img src="../../../assets/settings.svg" alt="No tag" class="empty-list-display__img" />
        </template>
      </EmptyListDisplay>
    </template>

    <v-dialog v-model="tagDialog" :width="getDialogWidth()" :fullscreen="display.smAndDown">
      <TagDialog :type :is-dialog-open="tagDialog" @create="createTag" @close="tagDialog = false" />
    </v-dialog>
  </div>
</template>

<style scoped lang="scss">
.tag-wrapper {
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(max(calc((100% / 4) - 3 * 4px), 200px), 1fr));
  row-gap: 8px;
  column-gap: 4px;
}

.empty-list-display {
  flex-grow: 1;

  &__img {
    width: clamp(200px, 50%, 300px);
  }
}
</style>
