<template>
  <v-container>
    ...
    <v-row>
      <v-col v-for="(dailyTaskOverview, index) in dailyTaskOverviewList" :key="index" cols="2">
        <DailyTaskOverviewItemCard :dailyTaskOverview="dailyTaskOverview" @openDailyTaskDetailDialog="openDailyTaskDetailDialog(dailyTaskOverview.date)">
    ...
    <v-dialog v-model="dailyTaskDialog" fullscreen hide-overlay scrollable transition="dialog-bottom-transition">
      <DailyTaskDetail :date="dateSelected" @dailyTaskCompleted="updateDailyTaskCompleted"
...
export default class DailyTaskOverview extends Vue {
  ...
  private updateDailyTaskCompleted(date: string, numberOfDailyTaskCompleted: number): void {
    const dailyTaskOverview = this.dailyTaskOverviewList.find((d: DailyTaskOverviewModel) => d.date === date);
