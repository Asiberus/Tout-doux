<template>
  <div class="wrapper">
    ...
    <div class="content">
      ...
      <v-timeline dense>
        <v-timeline-item v-for="dailyTask in dailyTaskList" :key="dailyTask.id" fill-dot
                         :color="dailyTask.completed ? 'green' : 'grey'" :icon="dailyTask.completed ? 'mdi-check' : 'mdi-trophy'">
          <v-card @click="toggleDailyTaskCompleteState(dailyTask)" :color="dailyTask.completed ? 'green darken-4' : null">
            <v-card-text>
              <div class="d-flex align-center">
                <v-chip v-if="dailyTask.action" class="mr-3" :color="getActionChipColor(dailyTask.action)">
...
export default class DailyTaskDetail extends Vue {
  ...
  private emitDailyTaskCompletedEvent(): void {
    const numberOfDailyTaskCompleted = this.dailyTaskList.filter((dailyTask: DailyTaskModel) => dailyTask.completed).length;
