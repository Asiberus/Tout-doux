      ...
      <template v-if="collectionList.length">
        <v-row align-content="start" class="position-relative">
          <v-col v-for="collection in collectionList" :key="'collection-' + collection.id" cols="3"
            ...
            <v-card v-on="!collection.selected ? {click: () => selectCollection(collection)}: {}"
                    :disabled="collection.tasks.length === 0" :title="!collection.selected ? 'Open collection' : null">
              <v-progress-linear :value="percentageOfTaskCompleted(collection)" color="green accent-2"
                                 height="4">
              </v-progress-linear>
              <v-card-text>
                <div class="d-flex justify-space-between align-center collection-card-header">
                  <h3 class="white--text">
                    <span v-if="collection.selected" class="grey--text">Select task of the collection : </span>
                    {{ collection.name }}
                  </h3>
                  ...
                  <v-btn v-if="collection.selected" @click.stop="collection.selected = false" color="red" icon>
                ...
                <template v-if="collection.selected">
                  ...
                  <template v-if="taskUncompleted(collection).length">
                    <v-row align-content="start" class="task-wrapper">
                      <v-col v-for="task of taskUncompleted(collection)" :key="'collection-task-' + task.id"
                             cols="6">
...
export default class DailyTaskUpdateCollectionList extends Vue {
  ...
  get percentageOfTaskCompleted(): (collection: DailyTaskCollectionDisplayModel) => number {
    return (collection: DailyTaskCollectionDisplayModel) => (this.numberOfTasksCompleted(collection) / collection.tasks.length) * 100;
  }
