<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-3">
      ...
      <v-btn @click="createDailyTaskDisplayed = !createDailyTaskDisplayed"
             icon :color="createDailyTaskDisplayed ? 'accent': null">
        ...
        <v-hover v-slot="{ hover }" :key="'daily-task' + dailyTask.id + dailyTask.name">
          <v-card class="daily-task-card mb-3">
            <v-card-text>
                ...
                <div class="d-flex align-center">
                  ...
                  <v-slide-x-reverse-transition>
                    <div v-if="hover" class="daily-task-card-actions">
                      <v-btn v-if="!dailyTask.taskId" @click="toggleDailyTaskEditMode(dailyTask, true)"
              ...
              <template v-else>
                <v-form v-model="dailyTaskForm.valid" @submit.prevent="handleDailyTaskFormSubmit(dailyTask.id)">
                  ...
                  <v-card-actions class="d-flex justify-end">
                    <v-btn color="success" small :disabled="!dailyTaskForm.valid"
                           @click="handleDailyTaskFormSubmit(dailyTask.id)">
                      <v-icon>mdi-check</v-icon>
                    </v-btn>
                    <v-btn color="error" small class="ml-1" @click="toggleDailyTaskEditMode(dailyTask, false)">
