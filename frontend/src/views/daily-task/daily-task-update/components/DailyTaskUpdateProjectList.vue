            ...
            <template v-if="projectList.length">
                <v-row align-content="start" class="position-relative">
                    <v-col
                        ...
                        <v-card
                            v-on="!project.selected ? { click: () => selectProject(project) } : {}"
                            :disabled="project.tasks.length === 0"
                            :title="!project.selected ? 'Open project' : null">
                            <v-progress-linear
                                :value="percentageOfTaskCompleted(project)"
                            ...
                            <v-card-text>
                                <div
                                    ...
                                    ">
                                    <h3 class="white--text">
                                        <span v-if="project.selected" class="grey--text"
                                            >Select task of the project :
                                        </span>
                                        {{ project.name }}
                                    </h3>
                                    <div class="mx-3" v-if="!project.selected">
                                        <span style="font-size: 1.8em" class="white--text">{{
                                            numberOfTasksCompleted(project)
                                        }}</span>
                                        /
                                        <span
                                            style="
                                    ...
                                    <v-btn
                                        ...
                                        @click.stop="project.selected = false"
                                        color="red"
                                        icon>
                                ...
                                <template v-if="project.selected">
                                    ...
                                    <template v-if="taskUncompleted(project).length">
                                        <v-row align-content="start" class="task-wrapper">
                                            <v-col
                                                v-for="task of taskUncompleted(project)"
                                    ...
                                    <template v-else>
                                        <EmptyListDisplay
                                            message="No task are related to this project">
                                            <template #img>
                                                <img
...
export default class DailyTaskUpdateProjectList extends Vue {
    ...
    get taskUncompleted(): (item: DailyTaskProjectDisplayModel) => TaskModel[] {
        return (item: DailyTaskProjectDisplayModel) =>
            item.tasks.filter((task: TaskModel) => !task.completed)
    }
    ...
    get numberOfTasksCompleted(): (item: DailyTaskProjectDisplayModel) => number {
        return (item: DailyTaskProjectDisplayModel) =>
            item.tasks.filter((task: TaskModel) => task.completed).length
    }

    get percentageOfTaskCompleted(): (item: DailyTaskProjectDisplayModel) => number {
        return (item: DailyTaskProjectDisplayModel) =>
            (this.numberOfTasksCompleted(item) / item.tasks.length) * 100
    }
