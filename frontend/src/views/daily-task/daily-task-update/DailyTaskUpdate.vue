<template>
  <v-container>
    <h1 class="mb-6"><span class="grey--text">Update Daily Task : </span>{{ dateFormatted }}</h1>

    <v-row>
      <v-col cols="8">
        <DailyTaskUpdateProjectList :projectList="projectList" :dailyTaskList="dailyTaskList"
                                    @selectTask="createDailyTask">
        </DailyTaskUpdateProjectList>

        <v-divider class="my-3"></v-divider>

        <DailyTaskUpdateCollectionList :collectionList="collectionList" :dailyTaskList="dailyTaskList"
                                       @selectTask="createDailyTask">
        </DailyTaskUpdateCollectionList>
      </v-col>
      <v-col cols="4" class="daily-task-wrapper">
        <DailyTaskUpdateList :dailyTaskList="dailyTaskList" @createDailyTask="createDailyTask"
                             @updateDailyTask="updateDailyTask" @deleteDailyTask="deleteDailyTask">
        </DailyTaskUpdateList>
...
import DailyTaskUpdateCollectionList
  from "@/views/daily-task/daily-task-update/components/DailyTaskUpdateCollectionList.vue";
...
export default class DailyTaskUpdate extends Vue {
  ...
  private updateDailyTask(dailyTaskId: number, dailyTaskForm: Partial<DailyTaskModel>): void {
    dailyTaskService.updateDailyTask(dailyTaskId, dailyTaskForm).then(
        (response: any) => {
          const dailyTask = this.dailyTaskList.find((dailyTask: DailyTaskDisplayModel) => dailyTask.id === response.body.id);
  ...
  private deleteDailyTask(dailyTaskId: number): void {
    dailyTaskService.deleteDailyTask(dailyTaskId).then(
        () => {
          const dailyTaskIndex = this.dailyTaskList.findIndex((dailyTask: DailyTaskDisplayModel) => dailyTask.id === dailyTaskId);
          if (dailyTaskIndex !== -1) {
