...
export class ProjectModule extends VuexModule {
    ...
    @Mutation
    private [projectMutations.updateProperties](payload: { name: string; description: string; archived: boolean }): void {
    ...
    @Action
    async [projectActions.updateProperties](payload: { id: number; data: Partial<ProjectModel> }): Promise<void> {
        const { id, data } = payload;
        projectService.updateProject(id, data).then(
            (response: any) => {
                const { name, description, archived } = response.body;
                this.context.commit(projectMutations.updateProperties, { name, description, archived });
            },
            (error: any) => {
                console.error(error)
            }
    ...
    @Action
    async [projectActions.section.editSection](payload: { id: number; name: string }): Promise<void> {
        const { id, name } = payload;
        await sectionService.updateSection(id, { name }).then(
            (response: any) => {
                this.context.commit(projectMutations.section.editSection, { id, name: response.body.name });
            }, (error: any) => {
                console.error(error);
            }
        )
    ...
    @Action
    async [projectActions.task.editTask](payload: { id: number; taskForm: Partial<TaskModel> }): Promise<void> {
    ...
    @Action
    async [projectActions.task.deleteTask](payload: { id: number; sectionId?: number }): Promise<void> {
